...
def dc3(in_list, src_list, point_orig,list_len,count):  # lcy:修改，加了src_list，point_orig，因为在递归时使用全局会影响结果
    b1_b2_list = []
    for i, point in enumerate(in_list):
        if point.val != in_list[-1].val and i % 3 in (1, 2):
    ...
    if curr_rank != len(b1_b2_list):  # 表明有重复 rank, 即排序未完成, 递归 dc3
        ...
        b1_b2_list = dc3(b1_b2_list, src_list_branch, point_orig_branch,list_len,count)  # lcy:递归，添加src-list_branch

    # 这里 b1_b2 就已经排序完了, 并且有了独特的 val
    # 利用 b1_b2 的 val 可以排序 b_0
    b_0_list = []
    for i, point in enumerate(in_list):
        if point.val != in_list[-1].val and i % 3 == 0:
...
def dc3_digui_input(src_list):
    ...
    result = dc3(point_list, src_list, point_orig,list_len,count)
    for i in result:
